...
- The same file now also listens for runtime messages from the background (and pulls any queued selections on load) so context-menu highlights auto-create a chat (if needed), write into `PromptInput`, and focus the field for user review (without auto-submitting); it also surfaces attached-context chips when the new “Attach As Context” entry runs, merging those snippets into the next submission.
...
- Conversations render with `Branch`, `BranchMessages`, `Message`, and `MessageContent` (all from `src/components/ai-elements`). `MessageParts` (in `src/components/ai-elements/message-parts.tsx`, used from `App.jsx`) is responsible for chain-of-thought, tool-call, text, and image rendering.
...
- `src/hooks/useModelConfig.js` centralizes model selection and preferences (`selectedModelId`, `modelPreferences`, `customModels`, and `enabledModelIds`) so `App.jsx` can stay focused on wiring while the hook keeps the enabled-model list and custom-model behavior consistent.
...
- `convertPartsToChainOfThought` inside `src/components/ai-elements/message-parts.tsx` converts tool progress + reasoning into a synthetic `chain-of-thought` part so the UI can collapse/expand the steps.
- `MessageParts` handles:
