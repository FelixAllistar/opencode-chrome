...
- Helper hooks now encapsulate most of App's side effects. `src/hooks/useApiKeyInputs.js` keeps the editable API-key drafts in sync with storage, `src/hooks/useProviderRegistrations.js` registers Brave/Context7 keys, `src/hooks/useContextSelectionBridge.js` drains pending chrome selections, `src/hooks/useUnhandledRejectionHandler.js` attaches the unhandled rejection guard, and `src/hooks/useChatBootstrap.js` bootstraps `chatsData`/`currentChatId` and input focus so App stays a simple composer that mostly renders UI.
