...
- The header pairs a `SidebarTrigger`, a “new chat” button (uses `AppSidebar`/`Sidebar` from `components/ui`), and a `SettingsMenu` (in `src/components/settings/SettingsMenu.jsx`) that surfaces the `ThemeSwitcher`, API key inputs, the tool toggles, and a “Clear All Chats” action. The menu now uses a shadcn `<Form />` stack backed by `react-hook-form`/`zod`; hitting Enter submits all visible key inputs together, the focused field flashes briefly when the save completes, and the hidden submit button ensures natural keyboard behavior while the tool switches persist through `enabledTools`.
